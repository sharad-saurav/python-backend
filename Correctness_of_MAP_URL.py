...
def rule_map_url(fle, fleName, target):
	import re
	import os
	import sys
	import json
			...
			for column_name in columns_to_apply:
				...
				if(pd.notnull(row[column_name])):
					if(validators.url(str(column_value))!=True):
						entry=[index,fleName,column_value+' is not have a valid MAP_URL']
						print('The row '+str(index)+' in the file '+fleName+' does not a valid Map url in '+column_name+' column')
						data.append(entry)
